        ...
        Column(modifier = Modifier.fillMaxSize().statusBarsPadding().navigationBarsPadding()) {
            ...
            Column(
                ...
                GlassCard(modifier = Modifier.fillMaxWidth()) {
                    Row(modifier = Modifier.padding(20.dp), verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Column {
                            ...
                            Spacer(Modifier.height(4.dp))
                            Text("$done of $total completed", style = MaterialTheme.typography.bodyMedium, color = SubtleText)
                            Spacer(Modifier.height(6.dp))
                            LinearProgressIndicator(
                                progress   = { if (total == 0) 0f else done.toFloat() / total },
                                modifier   = Modifier.width(160.dp).height(6.dp).clip(RoundedCornerShape(50)),
                                color      = AuroraMint,
                ...
                state.todayActions.forEach { action ->
                    ActionItem(action = action, onToggle = { viewModel.toggleActionComplete(action.id) })
    ...
    GlassCard(modifier = Modifier.fillMaxWidth()) {
        Row(modifier = Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
            Box(
                ...
                if (action.isCompleted) Text("✓", color = DeepIndigo, style = MaterialTheme.typography.labelLarge)
            }
            Spacer(Modifier.width(14.dp))
            Column(modifier = Modifier.weight(1f)) {
                Text(action.title, style = MaterialTheme.typography.bodyLarge, color = if (action.isCompleted) SubtleText else OnDark)
                Spacer(Modifier.height(4.dp))
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    CategoryChip("", action.type.label, accent)
                    action.durationMinutes?.let { CategoryChip("⏱", "${it}m", SubtleText) }
                }
            }
        }
    }
