            ...
            Column(
                ...
                GlassCard(modifier = Modifier.fillMaxWidth()) {
                    Row(
                        ...
                        HabitStat("${state.habits.maxOfOrNull { it.streak } ?: 0}", "Best streak", StarGold)
                        HabitStat("${state.habits.sumOf { it.streak }}", "Total days", SoftLavender)
                    }
                }
                ...
                state.habits.forEach { habit ->
                    HabitRow(habit = habit, onToggle = { viewModel.toggleHabitToday(habit.id) })
        ...
        Row(modifier = Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
            Box(
                modifier = Modifier
                    ...
                    .border(1.5.dp, if (habit.isCompletedToday) AuroraMint else SubtleText, CircleShape)
                    .clickable(onClick = onToggle),
                contentAlignment = Alignment.Center,
            ) {
                if (habit.isCompletedToday) Text("✓", color = DeepIndigo, style = MaterialTheme.typography.labelLarge)
            }
            ...
            if (habit.streak >= 7) {
                Surface(shape = RoundedCornerShape(50), color = StarGold.copy(alpha = 0.2f)) {
                    Text(
                        "🏆 ${habit.streak}d",
